...
//Prueba Unitaria de obtener la cantidad de chistes que hay en la base de datos por su categoría. Andrea Torres. Endpoint #6.
describe('GET /api/getJokesByCategory/:category', () => {
    beforeEach(async () => {
        ...
        it('debería devolver un mensaje de error 400 si la categoría no es válida', async () => {
            const response = await request(app)
                .get('/api/getJokesByCategory/invalidCategory')
                .expect(400);   
            expect(response.body.error).toBe('Categoría no válida. Las categorías permitidas son: Dad joke, Humor Negro, Chistoso o Malo.');
        });
            it('debería devolver un mensaje de error 500 si hay un error inesperado', async () => {
                ...
                const response = await request(app)
                    .get('/api/getJokesByCategory/Chistoso')
                    .expect(500);
...
//Prueba Unitaria de obtener todos los chistes que hay en la base de datos por puntaje de que tan bueno es, se tiene que pasar parámetro por URL. Andrea Torres. Endpoint #7.
describe('GET /api/getJokesByRating/:rating', () => {
    beforeEach(async () => {
        ...
        jokes.forEach(async (joke) => await joke.save());
    });
